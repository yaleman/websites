...
function parseContentIdFromLocation(
	...
	const match = location?.match(
		new RegExp(`/admin/site/${siteId}/content/([^/?]+)`),
	);
	if (!match) {
		throw new Error(
			`failed to parse content id from location: ${location ?? "(missing)"}`,
		);
	}
	return match[1];
}
...
function findAuditEvent(
	...
	const match = events.find(
		(event) =>
			...
			(criteria.entityId === undefined ||
				event.entityId === criteria.entityId) &&
			(criteria.entityType === undefined ||
				event.entityType === criteria.entityType),
	);
	if (!match) {
		throw new Error(
	...
	test("membership changes record the acting user in audit events", async () => {
		...
		try {
			...
			try {
				...
				const created = findAuditEvent(
					await listAuditEvents(harness, harness.siteId),
					{
						actorSub: actor,
