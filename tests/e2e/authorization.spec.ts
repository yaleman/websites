...
const routeCases: RouteCase[] = [
  ...
  },
  {
    ...
    prepare: async (harness, fixtures) => ({
      ...
      assertSuccess: async (response) => {
        ...
        expect(payload.assets.some((asset: { original_filename: string }) => (
  ...
  },
  {
    ...
    prepare: async (harness, _fixtures, scenario) => ({
      ...
      assertSuccess: async (response) => {
        expect(response.headers()["location"]).toBe(`/admin/site/${harness.siteId}/assets`);
      },
    }),
  },
  {
  ...
  },
  {
    ...
    prepare: async (harness, _fixtures, scenario) => ({
      ...
      assertSuccess: async (response) => {
        expect(response.headers()["location"]).toBe(`/admin/site/${harness.siteId}/tags`);
      },
    }),
  },
  {
  ...
  },
  {
    ...
    prepare: async (harness, fixtures) => ({
      ...
      assertSuccess: async (response) => {
        expect(await response.text()).toContain(fixtures.membershipTargetSubject);
      },
    }),
  },
  {
    ...
    test(`${route.method} ${route.name}`, async () => {
      ...
      try {
        ...
        try {
          ...
          const response = await requestWithSetup(lowestContext.api, route, setup);
        ...
        if (route.lowerRole) {
          ...
          try {
            ...
            const response = await requestWithSetup(lowerContext.api, route, setup);
            expect(response.status()).toBe(401);
            expect(await response.text()).toContain("does not satisfy required role");
          } finally {
        ...
        try {
          ...
          const response = await requestWithSetup(noMembershipContext.api, route, setup);
          expect(response.status()).toBe(401);
          expect(await response.text()).toContain(unauthorizedNoMembership(harness.siteId));
        } finally {
          await noMembershipContext.api.dispose();
        }

        const adminContext = await runAs(harness, actorCache, undefined, "admin");
        try {
          const setup = await route.prepare(harness, fixtures, "admin");
          const response = await requestWithSetup(adminContext.api, route, setup);
          expect(response.status()).toBe(route.successStatus);
          await setup.assertSuccess(response);
        } finally {
