...
function parseRevisions(stdout: string): RevisionRow[] {
  ...
  return lines.slice(1).map((line) => {
    const [id, revisionNumber, title, draft, createdAt] = line.split("\t");
    return {
      id,
...
export async function seedAuthorizationFixtures(
  ...
  const ownerMembershipId = await createMembership(harness, ownerUserId, "owner");
  ...
  const secondRevision = revisions.find((revision) => revision.revisionNumber === 2);
  if (!secondRevision) {
  ...
  const membershipTargetUserId = await createUser(harness, membershipTargetSubject);
